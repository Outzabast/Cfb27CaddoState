...
Two CLI utilities to snapshot the Caddo State Postgres database to a plain-SQL
file and load one back. Both run `pg_dump` / `psql` **inside the `caddo-db`
container**, so their version always matches the running Postgres 18 server (a
local `pg_dump` of a different major version would refuse to run).
...
Writes `backups/caddo-YYYYMMDD-HHMMSS.sql` (timestamped). The dump includes
`DROP ... IF EXISTS` + `CREATE` for every object plus all data, so it can be
restored over an existing database. The `backups/` folder is git-ignored; to
snapshot a specific dump into version control:

```bash
git add -f backups/caddo-YYYYMMDD-HHMMSS.sql
```
...
It prints the available backups if you don't pass a file. You'll be asked to type
the database name to confirm. To skip the prompt (e.g. in a script):

```bash
npm run db:restore -- backups/caddo-YYYYMMDD-HHMMSS.sql --yes
```
